...
public:
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        ...
        //parse tree
        answer = parseOrder(preorder, 0, lastIndex, inorder, 0, lastIndex);
        return answer;
    }

    TreeNode* parseOrder(vector<int>& preorder, int preS, int preE, vector<int>& inorder, int inS, int inE) {
        if (preS > preE || inS > inE) return nullptr;
        TreeNode* root = new TreeNode(preorder[preS]);

        int rootIndexInOrder = distance(inorder.begin(), std::find(inorder.begin(), inorder.end(), preorder[preS]));
        int noOfLeftTreeNodes = rootIndexInOrder - inS;
        //parse left tree only if more elements exist
        root -> left = parseOrder(preorder, preS + 1, preS + noOfLeftTreeNodes, inorder, inS, rootIndexInOrder - 1);

        //parse right tree
        root -> right = parseOrder(preorder, preS + noOfLeftTreeNodes + 1, preE, inorder, rootIndexInOrder + 1, inE);
